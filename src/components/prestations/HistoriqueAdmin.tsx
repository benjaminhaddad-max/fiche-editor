...
import { ClipboardCheck, FileSignature, UserPen, UserPlus } from 'lucide-react'
...
const ORIGINE: Record<Origin, { label: string; style: string; Icon: typeof UserPen }> = {
  ...
  provider: { label: 'Déclaré par le prestataire', style: 'bg-amber-50 text-amber-800 ring-amber-200', Icon: UserPlus },
  order: { label: 'Bon de mission', style: 'bg-sky-50 text-sky-700 ring-sky-200', Icon: ClipboardCheck },
}
  ...
  return (
    <>
      <div className="mb-6 grid gap-4 sm:grid-cols-3">
        ...
        <StatTile
          label="Déclarés par les prestataires"
        ...
        <Card className="mb-6 p-5">
          ...
          <div className="flex flex-wrap gap-2">
            {recap.map(([id, r]) => (
              <Link
                key={id}
                href={`/validation/historique?prestataire=${id}`}
